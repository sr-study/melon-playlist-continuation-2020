    ...
    def ensemble(self):
        ...
        for cur_res_list in zip(*results):
            ...
            if problem_type == ProblemType.SONG_ONLY:
                ensembled_songs = self.ensemble_two_lists(song_lists, 100, 'song')
                ensembled_tags = tag_lists[1]
            elif problem_type == ProblemType.SONG_TAG:
                ensembled_songs = self.ensemble_two_lists(song_lists, 100, 'song')
                ensembled_tags = self.ensemble_two_lists(tag_lists, 10, 'tag')
...
ensembler = Ensembler(['./activation_result/results.json', './penalty/results_penalty.json'], './res/val.json')
